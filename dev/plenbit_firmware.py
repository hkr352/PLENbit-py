...
class PLENbit():
    def __init__(self):
        ...
        # print("micro:bit for PLEN2 mini")
        # print("FirmWareVer:180927")
    ...
    def servoWrite(self, num, degrees):
        ...
        if debugFlag: print("write")
    def write8(self, addr, d):
        if debugFlag: print("init")
        cmd = bytearray(2)
        cmd[0] = addr
        cmd[1] = d
        self._i2c.write(0x6A, cmd)
        del cmd
        if debugFlag: print("init2")
...
uart.init(baudrate=115200, tx=pin8, rx=pin12)   #to BLE, comment = to PC
#pin16.write_digital(0)  #RS485 !RE/DE
pin16.write_digital(0) #EYE LED ON
Lsensor = pin0
Rsensor = pin2
    ...
    while 1:
        ...
        if((_buf != b"$") and (_buf != b"#")):
            ...
            if (_buf == b"PM"):
                ...
                display.show("M")
                #_int = int(_buf[0])
                #if ((102 >= _int) and (_int >= 97)) or ((57 >= _int) and (_int >= 48)) :
                try:
                    plenbit.motion(int(_buf, 16))
                except :
                    1+1
                    #print("Out of range")
                _buf = uart.read(40)
